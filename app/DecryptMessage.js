...
export default class DecryptMessage {
    ...
    async perform() {
        ...
        if (!key) { throw new Error('Could not find a key for this message') }

        return this.decryptWithKey(key)
    }

    async getPgpMessage() {
        if (!this._pgpMessage) { this._pgpMessage = await pgp.message.readArmored(this.ciphertext) }
    ...
    async pickKey() {
        if (!this._key) {
            ...
            if (!keyId) { throw new Error('Invalid message') }
    ...
    async decryptWithKey(key) {
        ...
        if (!private_) { throw new Error('No private key') }
